...
public class SQLStatisticsManager extends SQLManager {
    ...
    //Total summery
    public static List<Object[]> getTotalStatics(int flag, String sDate, String eDate)throws SQLException{
        List <Object[]> rows = new ArrayList<>();
        ResultSet results = null;
        String query1 = "select sum(t.amount) as Amount , sum(t.count) as Count , sum(t.amount - (s.buying_price * t.count)) as Profit, sum(s.buying_price * t.count) as Cost\n" +
            "from transactions t\n" +
            "inner join stock s on t.sid = s.sid ";
        String query2 = "select sum(t.amount) as Amount , sum(t.count) as Count , sum(t.amount - (s.buying_price * t.count)) as Profit, sum(s.buying_price * t.count) as Cost\n" +
            "from transactions t\n" +
            "inner join stock s on t.sid = s.sid \n" +
    ...
    public static List<Object[]> getOverallAmount(int flag ,String sDate, String eDate) throws SQLException{
        List <Object[]> rows = new ArrayList<>();
        ResultSet results = null;
        String query1 = "SELECT c.cid AS 'Category id', c.cname AS 'Category name', SUM(t.amount) AS 'Total amount'\n" +
            "FROM category c\n" +
            "LEFT JOIN stock s ON c.cid = s.cid\n" +
            "LEFT JOIN transactions t ON s.sid = t.sid\n" +
            "GROUP BY c.cid, c.cname;";
        
        String query2 = "SELECT c.cid AS 'Category id', c.cname AS 'Category name', SUM(t.amount) AS 'Total amount'\n" +
            "FROM category c\n" +
    ...
    public static List<Object[]> getSizeAmount(int flag ,String sDate, String eDate)throws SQLException{
        List <Object[]> rows = new ArrayList<>();
        ResultSet results = null;
        String query1 = "SELECT c.cid AS 'Category id', c.cname AS 'Category name', s.size AS 'Size', SUM(t.amount) AS 'Amount'\n" +
            "FROM category c\n" +
            "LEFT JOIN stock s ON c.cid = s.cid\n" +
            "LEFT JOIN transactions t ON s.sid = t.sid\n" +
            "GROUP BY c.cid, c.cname, s.size;"; 
        String query2 = "SELECT c.cid AS 'Category id', c.cname AS 'Category name', s.size AS 'Size', SUM(t.amount) AS 'Amount'\n" +
            "FROM category c\n" +
            "LEFT JOIN stock s ON c.cid = s.cid\n" +
            "LEFT JOIN transactions t ON s.sid = t.sid\n" +
            "WHERE t.date BETWEEN ? AND ?\n" +
    ...
    public static List<Object[]> getStockColorAmount(int flag ,String sDate, String eDate)throws SQLException{
        List <Object[]> rows = new ArrayList<>();
        ResultSet results = null;
        String query1 = "SELECT s.sid AS 'Stock id', c.cid AS 'Category id', c.cname AS 'Category name', s.size AS 'Size', clr.color AS 'Color', t.amount AS 'Amount'\n" +
            "FROM stock s\n" +
            "LEFT JOIN colors clr ON s.color_id = clr.color_id\n" +
            "LEFT JOIN category c ON s.cid = c.cid\n" +
            "LEFT JOIN transactions t ON s.sid = t.sid;";
        String query2 = "SELECT s.sid AS 'Stock id', c.cid AS 'Category id', c.cname AS 'Category name', s.size AS 'Size', clr.color AS 'Color', t.amount AS 'Amount'\n" +
            "FROM stock s\n" +
            "LEFT JOIN colors clr ON s.color_id = clr.color_id\n" +
            "LEFT JOIN category c ON s.cid = c.cid\n" +
            "LEFT JOIN transactions t ON s.sid = t.sid\n" +
    ...
    public static List<Object[]> getOverallCount(int flag ,String sDate, String eDate) throws SQLException{
        ResultSet results = null;
        List <Object[]> rows = new ArrayList<>();
        String query1 = "SELECT c.cid AS 'Category id',c.cname AS 'Category name',SUM(t.count) AS 'Total quantity'\n" +
            "FROM category c\n" +
            "LEFT JOIN stock s ON c.cid = s.cid\n" +
            "LEFT JOIN transactions t ON s.sid = t.sid\n" +
            "GROUP BY c.cid, c.cname;";
    ...
    public static List<Object[]> getSizeCount(int flag ,String sDate, String eDate)throws SQLException{
        ResultSet results = null;
        List <Object[]> rows = new ArrayList<>();
        String query1 = "SELECT c.cid AS 'Category id', c.cname AS 'Category name', s.size AS 'Size', SUM(t.count) AS 'Quantity'\n" +
            "FROM category c\n" +
            "LEFT JOIN stock s ON c.cid = s.cid\n" +
            "LEFT JOIN transactions t ON s.sid = t.sid\n" +
            "GROUP BY c.cid, c.cname, s.size;";
    ...
    public static List<Object[]> getStockColorCount(int flag ,String sDate, String eDate)throws SQLException{
        ResultSet results = null;
        List <Object[]> rows = new ArrayList<>();
        String query1 = "select s.sid AS 'stock id',c.cid as 'category id',c.cname as 'category name',s.size AS 'Size',clr.color as 'color',sum(t.count) as 'total quantity'\n" +
            "from category c\n" +
            "left join stock s on c.cid = s.cid\n" +
    ...
    public static List<Object[]> getOverallProfit(int flag ,String sDate, String eDate)throws SQLException{
        List <Object[]> rows = new ArrayList<>();
        ResultSet results = null;
        String query1 = "SELECT c.cid AS 'Category id',c.cname AS 'Category name',SUM(t.amount - s.buying_price * t.count) AS 'Total profit'\n" +
            "FROM category c\n" +
            "LEFT JOIN stock s ON c.cid = s.cid\n" +
            "LEFT JOIN transactions t ON s.sid = t.sid\n" +
            "GROUP BY c.cid, c.cname;";
    ...
    public static List<Object[]> getSizeProfit(int flag ,String sDate, String eDate)throws SQLException{
        List <Object[]> rows = new ArrayList<>();
        ResultSet results = null;
        String query1 = "SELECT c.cid AS 'Category id', c.cname AS 'Category name', s.size AS 'Size', SUM(t.count) AS 'Quantity', SUM((t.amount - s.buying_price * t.count)) AS 'Profit'\n" +
            "FROM category c\n" +
            "LEFT JOIN stock s ON c.cid = s.cid\n" +
            "LEFT JOIN transactions t ON s.sid = t.sid\n" +
            "GROUP BY c.cid, c.cname, s.size;";
    ...
    public static List<Object[]> getColorProfit(int flag ,String sDate, String eDate)throws SQLException{
        List <Object[]> rows = new ArrayList<>();
        ResultSet results = null;
        String query1 = "SELECT c.cid AS 'Category id', c.cname AS 'Category name', s.size AS 'Size',clr.color as 'color', SUM(t.count) AS 'Quantity', SUM((t.amount - s.buying_price * t.count)) AS 'Profit'\n" +
            "FROM category c\n" +
            "LEFT JOIN stock s ON c.cid = s.cid\n" +
            "LEFT JOIN colors clr ON s.color_id = clr.color_id\n" +
            "LEFT JOIN transactions t ON s.sid = t.sid\n" +
    ...
    public static List<Object[]> getStockProfit(int flag ,String sDate, String eDate)throws SQLException{
        List <Object[]> rows = new ArrayList<>();
        ResultSet results = null;
